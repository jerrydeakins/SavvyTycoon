...
# Plot upgrades modify crop base stats; they do not replace crop stats.
# yield_multiplier: final_yield = crop.base_yield * multiplier
# growth_reduction_days: final_growth = max(1, crop.growth_days - reduction)
const PLOT_UPGRADES := {
    1: {
        "cost": 0,
