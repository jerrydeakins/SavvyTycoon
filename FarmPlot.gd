...
var required_growth_days: int = 2
...
func plant(crop: String) -> bool:
    ...
    var growth_reduction: int = int(upgrade_data.get("growth_reduction_days", 0))
    required_growth_days = max(1, base_growth_days - growth_reduction)
...
func advance_growth_day() -> void:
    ...
    if growth_day >= required_growth_days:
        state = State.READY
...
func get_growth_progress_text() -> String:
    if crop_name == "":
        return ""
    return "%d/%d дней" % [growth_day, required_growth_days]
